...
def generateDatabaseGeneralizedGettersForDBM():
    ...
    for _, tables in getAllDBAliasesWithTables(dbc):
        for t in tables:
            ...
            ## construct function
            functionsString += f'''\n\tdef {functionName}({argumentString}) -> List[{rowAnnotationName}]:\n\t\treturn _dbGetter("{t}", **locals())\n'''
    functionsString += '\n'
    ...
    with open(os.path.join(path, 'managers', 'databaseManager.py'), 'r') as f:
        for line in f:
            if not rowObjectImportDone and line.startswith('from managers._generatedDatabaseExtras.databaseRowObjects') and line.endswith('Row\n'):
                ...
                continue
            elif not basicGetsRegionDone and line.startswith('\t#region basic generic gets - AUTO-GENERATED SECTION'):
                basicGetsRegionStarted = True
                newDBMString += line
                newDBMString += '\t## MANUALLY TRIGGERED, AUTO-GENERATED BY generateDatabaseGeneralizedGettersForDBM() FROM utils/dbSupport.py ##\n'
                newDBMString += functionsString
                continue
            elif not basicGetsRegionDone and basicGetsRegionStarted:
                if line.startswith('\t#endregion basic generic gets - AUTO-GENERATED SECTION'):
                    basicGetsRegionStarted = False
