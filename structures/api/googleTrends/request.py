...
class GoogleAPI(object):
    ...
    def __init__(self, hl='en-US', tz=300, geo='', timeout=(5, 30), proxies='',
    ...
    def _get_data(self, url, method=GET_METHOD, trim_chars=0, **kwargs):
        ...
        if self.retries > 0 or self.backoff_factor > 0:
            retry = Retry(total=self.retries, read=self.retries,
                          ...
                          status_forcelist=GoogleAPI.ERROR_CODES,
        ...
        if method == GoogleAPI.POST_METHOD:
    ...
    def _tokens(self):
        ...
        widget_dict = self._get_data(
            url=GoogleAPI.GENERAL_URL,
            method=GoogleAPI.GET_METHOD,
    ...
    def interest_over_time(self):
        ...
        req_json = self._get_data(
            url=GoogleAPI.INTEREST_OVER_TIME_URL,
            method=GoogleAPI.GET_METHOD,
            trim_chars=5,
            params=over_time_payload,
        )
        # print('req', over_time_payload)
        # print('resp', req_json)
        ...
        for idx, kw in enumerate(self.kw_list):
            ...
            result_df.insert(len(result_df.columns), 
                            kw if len(self.kw_list) > 1 else interestColumn,
    ...
    def interest_by_region(self, resolution='COUNTRY', inc_low_vol=False,
        ...
        # parse returned json
        req_json = self._get_data(
            url=GoogleAPI.INTEREST_BY_REGION_URL,
            method=GoogleAPI.GET_METHOD,
            trim_chars=5,
    ...
    def related_topics(self):
        ...
        for request_json in self.related_topics_widget_list:
            ...
            req_json = self._get_data(
                url=GoogleAPI.RELATED_QUERIES_URL,
                method=GoogleAPI.GET_METHOD,
                trim_chars=5,
    ...
    def trending_searches(self, pn='united_states'):
        ...
        req_json = self._get_data(
            url=GoogleAPI.TRENDING_SEARCHES_URL,
            method=GoogleAPI.GET_METHOD,
            **self.requests_args
        )[pn]
    ...
    def today_searches(self, pn='US'):
        ...
        req_json = self._get_data(
            url=GoogleAPI.TODAY_SEARCHES_URL,
            method=GoogleAPI.GET_METHOD,
            trim_chars=5,
            params=forms,
            **self.requests_args
    ...
    def suggestions(self, keyword):
        ...
        req_json = self._get_data(
            url=GoogleAPI.SUGGESTIONS_URL + kw_param,
            params=parameters,
            method=GoogleAPI.GET_METHOD,
            trim_chars=5,
            **self.requests_args
    ...
    def get_historical_interest(self, keywords, year_start=2018, month_start=1,
        ...
        exitFlag = False
        expBackoffInitial = expBackoff = 0.01
        while True:
            # format date to comply with API call

            start_date_str = formatDate(start_date)
            date_iterator_str = formatDate(date_iterator)
            ...
            try:
                ...
                if DEBUG: print('week_df', week_df)

                ## if no data, add 0s frame instead
                if week_df.empty:
                    d = {}
                    for ds in range((date_iterator - start_date).days):
                        d[start_date + timedelta(days=ds)] = 0
                    d = pd.DataFrame.from_dict(d, orient='index', columns=[interestColumn])
                    df = df.append(d)
                else:
                ...
                start_date += delta
                date_iterator += delta
                expBackoff = expBackoffInitial
            except Exception as e:
                print(e)
                expBackoff *= 2
                print('New backoff time:',expBackoff)
                pass
...
if __name__ == '__main__':
    gapi = GoogleAPI()
    d = gapi.get_historical_interest(None)
    # print(d.to_dict())
    print(list(d.to_dict()[interestColumn].keys())[0].strftime('%Y-%m-%d'))
    print(type(d))