...
def analyzeNUMFormulae():
    ...
    for q in tqdm.tqdm(loadedQuarters, desc='Analyzing quarters'):
        ...
        companies = dbm.dbc.execute(f'SELECT DISTINCT s.adsh FROM {dbm.getTableString("dump_edgar_sub")} s JOIN {dbm.getTableString("dump_edgar_num")} n, edgar_sub_balance_status b ON s.adsh=n.adsh and s.adsh=b.adsh WHERE b.status=0 AND fy=? AND fp=? AND exchange IS NOT NULL AND coreg=\'\' AND qtrs=\'0\' AND n.version LIKE \'%gaap%\'', (q[:4], q[4:].upper())).fetchall()
        # print(len(companies))
        # print(q)
        # print('keyspread', keyspread)
        for c in tqdm.tqdm(companies, desc='Companies', leave=False):
            numValues = dbm.dbc.execute(f'SELECT * FROM {dbm.getTableString("dump_edgar_num")} WHERE adsh=?', (c.adsh,)).fetchall()
            ## map to tag names
            ddatedict = {}
            for n in numValues:
...
if __name__ == '__main__':
    ...
    with open(os.path.join(path, 'miscellaneous/financialtagging/outp.txt'), 'r') as f:
        ...
        deprecatedtags = dbm.dbc.execute(f'select distinct tag from {dbm.getTableString("dump_edgar_tag")} where abstract=0 and tlabel like \'%deprecated%\'').fetchall()
